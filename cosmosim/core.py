...
class CosmoSimClass(QueryWithLogin):
    ...
    def _login(self, username=None, password=None, store_password=False,
        ...
        if authenticated.status_code == 200:
            ...
            self.delete_job(jobid=soup.find("uws:jobid").get_text(),
                            squash=True) ## fixed it
    ...
    def run_sql_query(self, query_string, tablename=None, queue=None,
        ...
        soup = BeautifulSoup(result.content, "lxml")
        self.current_job = soup.find("uws:jobid").get_text()
    ...
    def _generate_schema(self):
        ...
        for i in range(len(data['databases'])):
            ...
            stool = str(data['databases'][i]['tooltip'])
            self.db_dict[sstr]['tooltip'] = stool
            ...
            for j in range(len(data['databases'][i]['tables'])):
                ...
                stool2 = data['databases'][i]['tables'][j]['tooltip']
                self.db_dict[sstr]['tables'][sstr2]['tooltip'] = stool2
                ...
                if filename:
                    ...
                    if format.upper() == 'CSV':
                        raw_table_data = self._request(
                            ...
                            auth=(self.username, self.password),
                            cache=cache).content
                        #print(raw_table_data.decode())
                        #print(raw_table_data.split('\n')[0])
                        raw_table_data = raw_table_data.decode()
